...
import { updateReminderEmailConsent } from '@/app/perfil/actions'
import { cn } from '@/lib/utils'

export function ReminderEmailToggle({ initialConsent }: { initialConsent: boolean }) {
  ...
  const toggle = () => {
    ...
    startTransition(async () => {
      const res = await updateReminderEmailConsent(next)
      if (res?.error) {
        setConsent(!next)
        setError(res.error)
      }
