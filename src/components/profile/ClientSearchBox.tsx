  ...
  return (
    <div ref={rootRef} className="relative w-full max-w-[220px]">
      ...
      <input
        ...
        className="w-full bg-offwhite/5 border border-offwhite/[0.09] text-offwhite font-body font-light text-[12px] pl-[32px] pr-3 py-[8px] outline-none rounded-none focus:border-gold/50 transition-colors placeholder:text-offwhite/25"
      />
      ...
      {showPanel && (
        <div className="absolute left-0 top-[calc(100%+8px)] z-30 w-[300px] bg-charcoal border border-offwhite/[0.14] py-2 max-h-[360px] overflow-y-auto">
          ...
          {!loading && hits.length === 0 && (
            <p className="px-4 py-3 font-body font-light text-[11px] text-offwhite/35 italic">{t.client.noResults(query.trim())}</p>
          )}
